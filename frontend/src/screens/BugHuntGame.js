        ...
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={[styles.container, { backgroundColor: theme.background }]}>
            ...
            <View style={styles.header}>
                ...
                <View style={{ alignItems: 'center' }}>
                    <Text style={{ color: theme.textSecondary, fontSize: 14 }}>SKOR</Text>
                    <Text style={{ color: theme.success, fontSize: 24, fontWeight: 'bold', fontFamily: Platform.OS === 'ios' ? 'Courier' : 'monospace' }}>{score}</Text>
                </View>
            ...
            {/* Game Area */}
            <View style={[styles.gameArea, { borderBottomColor: theme.card }]}>
                {/* Server */}
                <Animated.View style={[styles.serverBox, {
                    backgroundColor: serverFlashAnim.interpolate({
                        inputRange: [0, 1],
                        outputRange: ['transparent', theme.danger + '80']
                    })
                }]}>
                    ...
                    <Animated.View style={[styles.bugContainer, { 
                        ...
                        {bugState === 'dead' && (
                            <View style={{ position: 'absolute', top: -55, width: SCREEN_WIDTH * 0.8, alignItems: 'center', zIndex: 10 }}>
                                <View style={{ backgroundColor: theme.overlay, paddingHorizontal: 15, paddingVertical: 8, borderRadius: 12, minWidth: 100, alignItems: 'center' }}>
                                    <Text numberOfLines={1} adjustsFontSizeToFit style={{ color: theme.success, fontSize: 18, fontWeight: 'bold' }}>
                        ...
                        {isPerfect && combo > 1 && bugState === 'dead' && (
                            <Text style={{ position: 'absolute', top: -20, color: COLORS.accent_gold, fontWeight: 'bold', fontSize: 16 }}>PERFECT!</Text>
                        )}
                    </Animated.View>
                )}
            </View>
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginVertical: 15, paddingHorizontal: 20 }}>
                <View style={{ flexDirection: 'row' }}>
                    <TouchableOpacity 
                        style={[styles.replayBtn, { backgroundColor: theme.accent, paddingHorizontal: 15 }]} 
                    ...
                    <TouchableOpacity 
                        style={[styles.replayBtn, { backgroundColor: hintsRemaining > 0 ? theme.warning : theme.elevated, marginLeft: 10 }]} 
                        onPress={handleHint}
                        disabled={hintsRemaining <= 0}
                    >
                        <Ionicons name="bulb" size={24} color={hintsRemaining > 0 ? theme.text : theme.textMuted} />
                        <Text style={{ color: hintsRemaining > 0 ? theme.text : theme.textMuted, fontWeight: 'bold', marginLeft: 5 }}>İpucu ({hintsRemaining})</Text>
                    </TouchableOpacity>
                </View>

                <View style={{ alignItems: 'center' }}>
                    <Text style={{ color: combo > 1 ? COLORS.accent_gold : theme.border, fontSize: 14, fontWeight: 'bold' }}>COMBO</Text>
                    <Text style={{ color: combo > 1 ? COLORS.accent_gold : theme.border, fontSize: 24, fontWeight: '900' }}>x{combo}</Text>
                </View>
            </View>

            {/* Typing Area */}
            <View style={[styles.typingArea, { backgroundColor: theme.background }]}>
                <View style={styles.typingBoxWrapper}>
                    {/* Görsel Kutucuk (Arkada) */}
                    <Animated.View style={[styles.displayBox, { backgroundColor: theme.card, borderColor: theme.card, transform: [{ translateX: shakeAnim }] }]}>
                        {currentWord && currentWord.word_en.split('').map((char, index) => {
                            const isTyped = index < inputText.length;
                            return (
                                <Text key={index} style={[styles.charText, { color: isTyped ? theme.success : theme.border }]}>
                                    {isTyped ? char : '_'}
                                </Text>
                            );
                        })}
                ...
                <Text style={{ color: theme.textMuted, marginTop: 15, fontSize: 12 }}>Böceği durdurmak için kelimeyi yaz!</Text>
            </View>
...
const styles = StyleSheet.create({
    ...
    gameArea: { flex: 1, justifyContent: 'center', borderBottomWidth: 2, borderBottomColor: COLORS.bg_card },
    ...
    soundWave: { position: 'absolute', top: -15, right: -15, backgroundColor: COLORS.accent_cyan, borderRadius: 10, padding: 2 },
    
    typingArea: { height: 180, alignItems: 'center', justifyContent: 'center', backgroundColor: COLORS.bg_primary },
    typingBoxWrapper: { position: 'relative', justifyContent: 'center', alignItems: 'center' },
    displayBox: { flexDirection: 'row', backgroundColor: COLORS.bg_card, padding: 20, borderRadius: 15, borderWidth: 2, borderColor: COLORS.bg_card, minWidth: 200, justifyContent: 'center' },
    ...
    replayBtn: { flexDirection: 'row', alignItems: 'center', backgroundColor: COLORS.accent_cyan, paddingVertical: 10, paddingHorizontal: 20, borderRadius: 20 },
