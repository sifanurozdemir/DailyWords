...
export default function ContextReviewScreen({ route, navigation }) {
    // HomeScreen artık pendingWords veya tekil bir word nesnesi gönderebilir
    const { pendingWords: paramsPendingWords, word: paramsWord, user_id } = route.params || {};
    const pendingWords = paramsPendingWords || (paramsWord ? [paramsWord] : []);
    const { theme, isDark } = useTheme();
    const { wordStates } = useWordKnowledge();

    const reviewList = React.useMemo(() => {
        if (!pendingWords || pendingWords.length === 0) return [];
        const sorted = [...pendingWords].sort((a, b) => {
            const masteryA = wordStates[a.id]?.mastery_level || 0;
            const masteryB = wordStates[b.id]?.mastery_level || 0;
            return masteryA - masteryB;
        });
        return sorted.slice(0, 10);
    }, [pendingWords, wordStates]);
    ...
    const word = reviewList && reviewList.length > 0 ? reviewList[currentIndex] : null;
    ...
    if (sessionComplete) {
        const isQuotaFilled = reviewList.length >= 10;
        return (
            <View style={[styles.center, { backgroundColor: theme.background, padding: 30 }]}>
                <Text style={{ fontSize: 90, marginBottom: 20 }}>🎉</Text>
                <Text style={{ color: theme.primary, fontSize: 26, fontWeight: '900', textAlign: 'center', marginBottom: 15 }}>
                    {isQuotaFilled ? "Harika! Kotanı doldurdun" : "Seans Tamamlandı!"}
                </Text>
                <Text style={{ color: theme.textSecondary, fontSize: 16, textAlign: 'center', marginBottom: 40, lineHeight: 24 }}>
                    {isQuotaFilled 
                        ? "İlk 10 kelimelik günlük tekrar kotanı başarıyla tamamladın. Devam etmek ister misin yoksa bugünlük bu kadar yeter mi?"
                        : `Bugünkü tüm ${reviewList.length} tekrarlarını başarıyla bitirdin ve kelimelerin hafızandaki yerini sağlamlaştırdın.`
                    }
                </Text>
                
                {pendingWords.length > 10 && (
                    <TouchableOpacity 
                        style={[styles.backBtn, { backgroundColor: COLORS.accent_purple, width: '100%', paddingVertical: 18, borderRadius: 25, elevation: 5, marginBottom: 15 }]}
                        onPress={() => {
                            const remaining = pendingWords.slice(10);
                            navigation.replace('ContextReview', { pendingWords: remaining, user_id });
                        }}
                    >
                        <Text style={[styles.backBtnText, { fontSize: 18, fontWeight: '900' }]}>10 TANE DAHA?</Text>
                    </TouchableOpacity>
                )}
                
                <TouchableOpacity 
                    style={[styles.backBtn, { backgroundColor: COLORS.accent_purple, width: '100%', paddingVertical: 18, borderRadius: 25, elevation: 5 }]}
                    onPress={() => {
                        navigation.navigate('Home', { refresh: Date.now() });
                    }}
                >
                    <Text style={[styles.backBtnText, { fontSize: 18, fontWeight: '900' }]}>BUGÜNLÜK BİTTİ</Text>
                </TouchableOpacity>
            </View>
        );
    }
    ...
    const progress = ((currentIndex) / reviewList.length) * 100;
            ...
            <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
                ...
                <View style={[styles.wordCard, { backgroundColor: theme.card, shadowColor: theme.shadow }]}>
                    <Text style={[styles.wordEn, { color: theme.text }]}>
                        {(!answered && question.question_type !== 'meaning') ? '❔' : question.word_en}
                    </Text>
                ...
                {answered && (
                    <View style={[styles.feedbackBox, { backgroundColor: 'transparent', padding: 0, marginTop: 10 }]}>
                        ...
                        <TouchableOpacity
                            style={[styles.backBtn, { backgroundColor: COLORS.accent_purple, width: '100%', paddingVertical: 18, borderRadius: 20 }]}
                            onPress={handleNext}
                        >
...
const styles = StyleSheet.create({
    ...
    backBtnText: { color: COLORS.text_primary, fontWeight: 'bold' },
});
