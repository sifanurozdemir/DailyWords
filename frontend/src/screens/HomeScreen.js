...
import { View, Text, StyleSheet, ScrollView, TouchableOpacity, ActivityIndicator, Modal, StatusBar, Switch, Platform, Animated, RefreshControl } from 'react-native';
...
export default function HomeScreen({ route, navigation }) {
    ...
    const [subTab, setSubTab] = useState('today'); // 'today', 'words', 'srs'
    ...
    useFocusEffect(
        ...
        }, [user?.user_id, words.length])
    );

    const onRefresh = useCallback(() => {
        setRefreshing(true);
        fetchInitialData(false).finally(() => setRefreshing(false));
    }, [user?.user_id]);
    ...
    const getMedal = (score, bronze = 5, silver = 10, gold = 15) => {
        if (score >= gold) return { icon: '🥇', label: 'Altın', color: COLORS.medal_gold, bg: COLORS.medal_gold + '26' };
        if (score >= silver) return { icon: '🥈', label: 'Gümüş', color: COLORS.medal_silver, bg: COLORS.medal_silver + '26' };
        if (score >= bronze) return { icon: '🥉', label: 'Bronz', color: COLORS.medal_bronze, bg: COLORS.medal_bronze + '26' };
        return { icon: '🔒', label: 'Kilitli', color: COLORS.text_muted, bg: 'transparent' };
    };
        ...
        <View style={[styles.outerContainer, { backgroundColor: theme.background }]}>
            ...
            {currentTab === 'home' && (
                <View style={styles.container}>
                    <TouchableOpacity onPress={openLearnedModal} activeOpacity={0.8}>
                        <View style={[styles.progressSection, { backgroundColor: theme.card }]}>
                            ...
                            <View style={[styles.progressBarBg, { backgroundColor: COLORS.border }]}>
                                <View style={[styles.progressBarFill, { width: `${Math.min((learnedCount / totalWords) * 100, 100)}%`, backgroundColor: COLORS.accent_purple }]} />
                            </View>
                    ...
                    <View style={[styles.subTabBar, { backgroundColor: theme.card }]}>
                        <TouchableOpacity 
                            style={[styles.subTabItem, subTab === 'today' ? styles.subTabItemActive : null, subTab === 'today' ? { backgroundColor: theme.primary } : null]} 
                            onPress={() => setSubTab('today')}
                        >
                            <Text style={[styles.subTabLabel, { color: subTab === 'today' ? COLORS.text_primary : theme.textSecondary }]}>Bugün</Text>
                        </TouchableOpacity>
                        <TouchableOpacity 
                            style={[styles.subTabItem, subTab === 'words' ? styles.subTabItemActive : null, subTab === 'words' ? { backgroundColor: theme.primary } : null]} 
                            onPress={() => setSubTab('words')}
                        >
                            <Text style={[styles.subTabLabel, { color: subTab === 'words' ? COLORS.text_primary : theme.textSecondary }]}>Kelimeler</Text>
                        </TouchableOpacity>
                        <TouchableOpacity 
                            style={[styles.subTabItem, subTab === 'srs' ? styles.subTabItemActive : null, subTab === 'srs' ? { backgroundColor: theme.primary } : null]} 
                            onPress={() => setSubTab('srs')}
                        >
                            <Text style={[styles.subTabLabel, { color: subTab === 'srs' ? COLORS.text_primary : theme.textSecondary }]}>SRS</Text>
                        </TouchableOpacity>
                    </View>
                    ...
                    {loading ? <ActivityIndicator size="large" color={theme.primary} style={{ marginTop: 30 }} /> : (
                        <View style={{ flex: 1 }}>
                            ...
                            {subTab === 'today' && (
                                ...
                                >
                                    {/* 1. ÖĞRENME GÖREVİ CARD */}
                                    <View style={[styles.taskCard, { backgroundColor: theme.card, borderColor: theme.border, borderWidth: 1 }]}>
                                        <Text style={[styles.taskHeader, { color: theme.text }]}>
                                            ÖĞRENME GÖREVİ — {Math.min(learnedToday, dailyGoal)} / {dailyGoal}
                                        </Text>
                                        
                                        {words.length > 0 ? (
                                            <View style={{ marginVertical: 10 }}>
                                                {words.slice(0, 5).map(item => (
                                                    <View key={`today-word-${item.id}`} style={styles.taskWordRow}>
                                                        <Text style={[styles.taskWordText, { color: theme.text }]}>{item.word_en}</Text>
                                                        <View style={[styles.taskBadge, { backgroundColor: COLORS.accent_green + '22' }]}>
                                                            <Text style={{ fontSize: 11, fontWeight: 'bold', color: COLORS.accent_green }}>yeni</Text>
                                                        </View>
                                                    </View>
                                                ))}
                                                {words.length > 5 && (
                                                    <Text style={{ color: theme.textMuted, fontSize: 12, marginTop: 5, textAlign: 'center' }}>
                                                        + {words.length - 5} kelime daha
                                                    </Text>
                                                )}
                                            </View>
                                        ) : (learnedToday >= dailyGoal && !isExtraMode) ? (
                                            <Text style={[styles.taskEmptyText, { color: COLORS.accent_green, marginVertical: 15 }]}>🏆 Bugünkü hedefini tamamladın!</Text>
                                        ) : (
                                            <Text style={[styles.taskEmptyText, { color: theme.textMuted, marginVertical: 15 }]}>Öğrenilecek yeni kelime yok.</Text>
                                        )}

                                        {learnedToday < dailyGoal && words.length > 0 && (
                                            <TouchableOpacity 
                                                style={[styles.taskActionBtn, { backgroundColor: COLORS.accent_purple }]}
                                                onPress={() => {
                                                    const listToStudy = studyList.length > 0 ? studyList : words;
                                                    navigation.navigate('Practice', { studyList: listToStudy, user_id: user.user_id });
                                                }}
                                            >
                                                <Text style={styles.taskActionBtnText}>Öğrenmeye Başla ➔</Text>
                                            </TouchableOpacity>
                                        )}
                                    </View>

                                    {/* 2. TEKRAR ZAMANI CARD */}
                                    {(() => {
                                        const pending = reviewWords.filter(w => !w.reviewed_today);
                                        const pendingCount = pending.length;
                                        const completedCount = reviewWords.length - pendingCount;
                                        
                                        let btnText = "Tekrarlara Başla ➔";
                                        let btnColor = COLORS.accent_amber;
                                        let isBtnDisabled = false;
                                        
                                        if (pendingCount === 0) {
                                            btnText = "✓ Bugün temizlendi";
                                            btnColor = COLORS.accent_green;
                                            isBtnDisabled = true;
                                        }

                                        return (
                                            <View style={[styles.taskCard, { backgroundColor: theme.card, borderColor: theme.border, borderWidth: 1, marginTop: 20 }]}>
                                                <Text style={[styles.taskHeader, { color: theme.text }]}>
                                                    TEKRAR ZAMANI — {completedCount} / {reviewWords.length}
                                                </Text>

                                                {pendingCount > 0 ? (
                                                    <View style={{ marginVertical: 10 }}>
                                                        {pending.slice(0, 5).map(item => (
                                                            <View key={`today-review-${item.id}`} style={styles.taskWordRow}>
                                                                <Text style={[styles.taskWordText, { color: theme.text }]}>{item.word_en}</Text>
                                                                <View style={[styles.taskBadge, { backgroundColor: COLORS.accent_purple + '22' }]}>
                                                                    <Text style={{ fontSize: 11, fontWeight: 'bold', color: COLORS.accent_purple }}>tekrar</Text>
                                                                </View>
                                                            </View>
                                                        ))}
                                                        {pendingCount > 5 && (
                                                            <Text style={{ color: theme.textMuted, fontSize: 12, marginTop: 5, textAlign: 'center' }}>
                                                                + {pendingCount - 5} kelime daha
                                                            </Text>
                                                        )}
                                                    </View>
                                                ) : (
                                                    <Text style={[styles.taskEmptyText, { color: COLORS.accent_green, marginVertical: 15 }]}>🎉 Bütün tekrarlar temizlendi!</Text>
                                                )}

                                                {!isBtnDisabled && (
                                                    <TouchableOpacity 
                                                        style={[styles.taskActionBtn, { backgroundColor: btnColor }]}
                                                        onPress={() => navigation.navigate('ContextReview', { pendingWords: pending, user_id: user.user_id })}
                                                        disabled={isBtnDisabled}
                                                    >
                                                        <Text style={styles.taskActionBtnText}>{btnText}</Text>
                                                    </TouchableOpacity>
                                                )}
                                            </View>
                                        );
                                ...
                                <View style={{ flex: 1 }}>
                                        ...
                                        {reviewWords.length > 0 ? (
                                            <View>
                                                {reviewWords.map(item => (
                                                    <React.Fragment key={`review-${item.id}`}>{renderReviewCard(item)}</React.Fragment>
                                                ))}
                                            </View>
                                        ) : (
                                            <View style={[styles.completedBox, { backgroundColor: theme.card }]}>
                                                <Text style={styles.completedEmoji}>🎉</Text>
                                                <Text style={[styles.completedTitle, { color: COLORS.accent_green }]}>Bütün tekrarlar temizlendi!</Text>
                                                <Text style={[styles.completedSub, { color: theme.textSecondary }]}>Bugün için beklemede olan başka tekrar kelimen yok.</Text>
                                            </View>
                                        )}
                                    </ScrollView>
                                    ...
                                    {(() => {
                                        ...
                                        if (pendingCount === 0) {
                                            ...
                                            btnColor = COLORS.accent_green;
                                            isBtnDisabled = true;
                                        } else if (pendingCount >= 1 && pendingCount <= 10) {
                                            btnText = `🔁 ${pendingCount} kelime bekliyor`;
                                            btnColor = COLORS.accent_cyan;
                                        } else if (pendingCount >= 11 && pendingCount <= 30) {
                                            btnText = "⚡ Hızlı tekrar";
                                            btnColor = COLORS.accent_amber;
                                        } else {
                                            btnText = "📚 10'la başla →";
                                            btnColor = COLORS.accent_amber;
                                        }
            ...
            {currentTab === 'games' && (
                ...
                >
                    ...
                    {/* 2. OYUN: SENTENCE BUILDER (AKTİF EDİLDİ) */}
                    <TouchableOpacity 
                        style={[styles.profileCard, { 
                            backgroundColor: theme.card, 
                            borderColor: theme.primary, 
                    ...
                    >
                        ...
                        <View style={{ flex: 1 }}>
                            <Text style={[styles.profileSectionTitle, { color: theme.text, marginBottom: 2 }]}>Sentence Builder</Text>
                            <Text style={{ color: theme.textSecondary, fontSize: 13 }}>
                                Karışık kelimelerden doğru cümleyi oluştur.
                            </Text>
                        </View>
                        <Ionicons name="chevron-forward" size={20} color={theme.primary} />
                    </TouchableOpacity>
            ...
            {currentTab === 'profile' && (
                <ScrollView contentContainerStyle={[styles.profileContent, { paddingBottom: 120 }]} showsVerticalScrollIndicator={false}>
                    ...
                    <View style={[styles.profileCard, { backgroundColor: COLORS.bg_card, borderColor: COLORS.border, borderWidth: 1 }]}>
                        ...
                        <View style={[styles.progressBarBg, { backgroundColor: COLORS.border, height: 16, borderRadius: 8 }]}>
                            <View style={[styles.progressBarFill, { width: `${currentLevelProgress}%`, backgroundColor: COLORS.accent_purple, borderRadius: 8 }]} />
                        </View>
                    </View>

                    {/* Ateş Serisi (Streak) */}
                    <View style={[styles.profileCard, { backgroundColor: COLORS.bg_card, borderColor: COLORS.border, borderWidth: 1, flexDirection: 'row', alignItems: 'center', padding: 20 }]}>
        ...
        <View style={[styles.card, { backgroundColor: theme.card, borderColor: isSelected ? theme.accent : theme.border }]}>
            <View style={styles.wordInfo}>
                <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 4 }}>
                    <View style={{
                        ...
                        height: 10,
                        borderRadius: 5,
                        backgroundColor: getMasteryColor(item.id),
                        marginRight: 8
                    }} />
                    <Text style={[styles.wordEn, { color: theme.text, marginBottom: 0 }]}>{item.word_en}</Text>
                </View>
            ...
            <View style={styles.actionRow}>
                {!isWaitingUndo && (
                    ...
                    >
                        <Text style={{ color: isSelected ? COLORS.text_primary : theme.text }}>{isSelected ? "📋 Seçildi" : "➕ Çalış"}</Text>
                    </TouchableOpacity>
                )}
                ...
                <TouchableOpacity
                    ...
                    style={[
                        ...
                        { 
                            ...
                            backgroundColor: isWaitingUndo ? COLORS.success : theme.primaryLight 
                        }
                    ]}
                    onPress={handlePress}
                    disabled={isSelected}
                >
                    ...
                    <Text style={[
                        ...
                        { 
                            ...
                            color: isWaitingUndo ? COLORS.text_primary : theme.successText,
                            zIndex: 11 
                        }
                    ]}>
...
const styles = StyleSheet.create({
    ...
    startBtnText: { color: COLORS.text_primary, textAlign: 'center', fontWeight: '900', fontSize: 17 },

    sessionBtn: { padding: 16, borderRadius: 16, marginTop: 15, elevation: 3, alignItems: 'center' },
    sessionBtnText: { color: COLORS.text_primary, textAlign: 'center', fontWeight: 'bold', fontSize: 16 },
    ...
    medalBox: { width: 60, height: 60, borderRadius: 15, justifyContent: 'center', alignItems: 'center', borderWidth: 1, borderColor: COLORS.border },
    ...
    toast: { position: 'absolute', top: 50, alignSelf: 'center', backgroundColor: COLORS.bg_card, paddingVertical: 12, paddingHorizontal: 24, borderRadius: 25, elevation: 5 },
    undoToast: { position: 'absolute', bottom: 100, alignSelf: 'center', backgroundColor: COLORS.bg_card, paddingVertical: 14, paddingHorizontal: 20, borderRadius: 30, elevation: 8, flexDirection: 'row', alignItems: 'center' },
    toastText: { color: COLORS.text_primary, fontSize: 14, fontWeight: 'bold' },
    undoBtn: { marginLeft: 14, backgroundColor: COLORS.accent_amber, paddingVertical: 6, paddingHorizontal: 12, borderRadius: 15 },
    undoBtnText: { color: COLORS.text_primary, fontWeight: 'bold' },

    subTabBar: { flexDirection: 'row', padding: 4, borderRadius: 14, marginHorizontal: 20, marginVertical: 15 },
    subTabItem: { flex: 1, paddingVertical: 10, borderRadius: 12, alignItems: 'center' },
    subTabItemActive: { elevation: 2, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.1, shadowRadius: 3 },
    subTabLabel: { fontSize: 14, fontWeight: 'bold' },

    taskCard: { borderRadius: 20, padding: 20, elevation: 2 },
    taskHeader: { fontSize: 16, fontWeight: '800', marginBottom: 12 },
    taskWordRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingVertical: 10, borderBottomWidth: 0.5, borderColor: COLORS.border },
    taskWordText: { fontSize: 16, fontWeight: '600' },
    taskBadge: { paddingHorizontal: 8, paddingVertical: 4, borderRadius: 8 },
    taskEmptyText: { fontSize: 14, fontStyle: 'italic', textAlign: 'center', marginVertical: 15 },
    taskActionBtn: { paddingVertical: 14, borderRadius: 16, alignItems: 'center', marginTop: 15 },
    taskActionBtnText: { color: COLORS.text_primary, fontWeight: 'bold', fontSize: 15 },
});