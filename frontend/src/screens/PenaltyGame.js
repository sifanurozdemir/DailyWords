                    ...
                    <View style={[styles.questionCard, { backgroundColor: theme.card, borderColor: theme.border }]}>
                        <View style={styles.timerRow}>
                            <View style={[styles.timerBarBg, { backgroundColor: theme.progressBg }]}>
                                <View style={[styles.timerBarFill, { width: `${(timeLeft/5)*100}%`, backgroundColor: timeLeft < 2 ? COLORS.game_wrong : theme.primary }]} />
                            </View>
                        ...
                        {options.map((opt, i) => {
                            ...
                            if (feedback) {
                                if (opt.id === currentWord.id) {
                                    borderColor = COLORS.game_correct; 
                                    borderWidth = 3;
                                } else if (feedback === 'miss' && opt.id === feedbackTargetId) {
                                    borderColor = COLORS.game_wrong;
                                    borderWidth = 3;
                                }
                            }
...
const styles = StyleSheet.create({
    ...
    scoreText: { color: COLORS.text_primary, fontWeight: 'bold' },
    ...
    netLine: { width: '100%', height: 1, backgroundColor: COLORS.text_secondary, marginTop: 35, opacity: 0.2, position: 'absolute' },
    ball: { zIndex: 5, marginBottom: 15 },
    ballShadow: { width: 45, height: 12, backgroundColor: COLORS.bg_primary, borderRadius: 20, position: 'absolute', bottom: 10, zIndex: 1 },
    particle: { position: 'absolute', fontSize: 30, zIndex: 10 },
    goalFlash: { position: 'absolute', top: 60, fontSize: 50, fontWeight: '900', color: COLORS.game_correct, zIndex: 20 },
    missFlash: { position: 'absolute', top: 60, fontSize: 40, fontWeight: '900', color: COLORS.game_wrong, zIndex: 20 },
    ...
    startBtnText: { color: COLORS.text_primary, fontSize: 20, fontWeight: 'bold' }
});