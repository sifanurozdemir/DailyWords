...
export default function PlacementTestScreen({ route, navigation }) {
    ...
    const { updateUserFields } = useUser();
    ...
    const submitGoal = async (goal) => {
        try {
            ...
            await updateUserFields({ current_level: detectedLevel, daily_goal: goal, needs_placement_test: false });
    ...
    if (showResultScreen) {
        ...
        return (
            <View style={[styles.container, styles.centerAll, { backgroundColor: theme.background }]}>
                ...
                <TouchableOpacity 
                    style={[styles.resultBtn, { backgroundColor: COLORS.accent_purple, shadowColor: COLORS.accent_purple }]} 
            ...
            <View style={styles.optionsWrapper}>
                {currentQ?.options.map((opt, index) => (
                    <TouchableOpacity
                        ...
                        style={[
                            styles.optionBtn,
                            { backgroundColor: COLORS.bg_elevated, borderColor: COLORS.border },
                            selectedOption ? (
                                opt.is_correct 
                                    ? { backgroundColor: COLORS.accent_purple + '33', borderColor: COLORS.accent_purple, borderWidth: 2 }
                                    : (selectedOption === opt 
                                        ? { backgroundColor: COLORS.danger + '33', borderColor: COLORS.danger, borderWidth: 2 }
                                        : { opacity: 0.5 })
                            ) : null
                        ]}
...
const styles = StyleSheet.create({
    ...
    resultBtnText: { color: COLORS.text_primary, fontSize: 18, fontWeight: '800' },

    overlay: { position: 'absolute', top: 0, bottom: 0, left: 0, right: 0, backgroundColor: COLORS.overlay_dark, justifyContent: 'center', alignItems: 'center', padding: 20, zIndex: 10 },
